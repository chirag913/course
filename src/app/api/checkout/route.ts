...
function resolveServerRazorpayKeyId(): string | undefined {
  return (
    normalizeEnv(process.env.RAZORPAY_KEY_ID) ??
...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      const message =
        error instanceof Error && /key_id|oauthToken|key_secret|credentials/i.test(error.message)
          ? "Payment provider is not configured. Please add RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET in deployment settings."
    ...
    const message =
      error instanceof Error && /key_id|oauthToken|key_secret|credentials/i.test(error.message)
        ? "Payment provider is not configured. Please add RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET in deployment settings."
        : "Something went wrong while creating checkout. Please try again.";
